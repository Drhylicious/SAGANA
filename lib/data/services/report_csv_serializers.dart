...
String serializeSalesReportCsv(SalesReportData data) {
  ...
  buffer.writeln(_row([
    'Farmer Name', 'Member ID', 'Crop Type', 'Crop Name',
    'Quantity (kg)', 'Amount (PHP)', 'Sale Date', 'Reference No',
  ]));
...
String serializeInventoryReportCsv(InventoryReportData data) {
  final buffer = StringBuffer();
  buffer.writeln(_row([
    'Farmer Name', 'Member ID', 'Crop', 'Batch Number', 'Quality Grade',
    'Available (kg)', 'Reserved (kg)', 'Sold (kg)', 'Status',
  ]));
...
String serializeHarvestReportCsv(HarvestReportData data) {
  final buffer = StringBuffer();
  buffer.writeln(_row([
    'Date', 'Farmer Name', 'Member ID', 'Crop', 'Quality Grade',
    'Quantity (kg)', 'Submitted to Coop', 'Synced',
  ]));
...
String serializeExpenseReportCsv(ExpenseReportData data) {
  final buffer = StringBuffer();
  buffer.writeln(_row([
    'Date', 'Farmer Name', 'Member ID', 'Category', 'Description',
    'Amount (PHP)', 'Subsidized',
  ]));
...
String serializeLoanReportCsv(List<AdminLoanSummary> loans) {
  final buffer = StringBuffer();
  buffer.writeln(_row([
    'Reference No', 'Farmer Name', 'Member ID', 'Issued Date', 'Items',
    'Total Value (PHP)', 'Amount Paid (PHP)', 'Remaining Balance (PHP)', 'Status',
  ]));
...
String serializeMemberContributionReportCsv(MemberContributionReportData data) {
  final buffer = StringBuffer();
  buffer.writeln(_row([
    'Farmer Name', 'Member ID', 'Palay Qty (kg)', 'Palay Amount (PHP)',
    'Peanut Qty (kg)', 'Peanut Amount (PHP)', 'Total Amount (PHP)', 'Share (%)',
  ]));
